...
class _PostsState extends State<Posts> {
  ...
  buildLikeButton() {
    return StreamBuilder(
      stream: likesRef.where('postId', isEqualTo: widget.post!.postId).where('userId', isEqualTo: currentUserId()).snapshots(),
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasData) {
...
//delete notification associated with that given post
    QuerySnapshot notificationsSnap =
        await notificationRef.doc(widget.post!.ownerId!).collection('notifications').where('postId', isEqualTo: widget.post!.postId).get();
...
//delete all the comments associated with that given post
    QuerySnapshot commentSnapshot = await commentRef.doc(widget.post!.postId).collection('comments').get();
